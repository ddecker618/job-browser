...
export class DiscoveryAnalyticsService {
  ...
  public getGlobalSummary(): GlobalDiscoverySummary {
    ...
    const enabledSources = db
      .prepare('SELECT COUNT(*) AS count FROM sources WHERE enabled = 1')
      .get() as { count: number };
    const disabledSources = db
      .prepare('SELECT COUNT(*) AS count FROM sources WHERE enabled = 0')
      .get() as { count: number };
    const totalCareerSites = db
      .prepare('SELECT COUNT(*) AS count FROM career_sites')
      .get() as { count: number };
    const activeCareerSites = db
      .prepare(
        "SELECT COUNT(*) AS count FROM career_sites WHERE discovery_state != 'retired'",
      )
      .get() as { count: number };
    const retiredCareerSites = db
      .prepare(
        "SELECT COUNT(*) AS count FROM career_sites WHERE discovery_state = 'retired'",
      )
      .get() as { count: number };
    const healthyCareerSites = db
      .prepare(
        "SELECT COUNT(*) AS count FROM career_sites WHERE health_status = 'healthy'",
      )
      .get() as { count: number };
    const warningCareerSites = db
      .prepare(
        "SELECT COUNT(*) AS count FROM career_sites WHERE health_status = 'warning'",
      )
      .get() as { count: number };
    const brokenCareerSites = db
      .prepare(
        "SELECT COUNT(*) AS count FROM career_sites WHERE health_status = 'broken'",
      )
      .get() as { count: number };
    const unknownCareerSites = db
      .prepare(
        "SELECT COUNT(*) AS count FROM career_sites WHERE health_status = 'unknown'",
      )
      .get() as { count: number };
  ...
  public getActivityWindow(
    ...
    if (durationRows.length > 0) {
      ...
      if (durationRows.length % 2 === 0) {
        medianDurationMs =
          ((durationRows[mid - 1]?.execution_time_ms ?? 0) +
            (durationRows[mid]?.execution_time_ms ?? 0)) /
          2;
      } else {
    ...
    return {
      ...
      averageDurationMs:
        stats.avg_duration === null ? null : Math.round(stats.avg_duration),
      medianDurationMs:
        medianDurationMs === null ? null : Math.round(medianDurationMs),
  ...
  public getJobYield(start: string, end: string): DiscoveryJobYield {
    ...
    return {
      ...
      newJobYieldPerSuccessfulRun:
        successfulRuns === 0 ? 0 : newCanonicalCount / successfulRuns,
  ...
  public getSourceAnalytics(): SourceAnalyticsRow[] {
    ...
    return sources.map((source) => {
      ...
      const staleDurationHours = runStats.last_success
        ? Math.max(0, now.getTime() - Date.parse(runStats.last_success)) /
          3600000
        : null;
      ...
      return {
        ...
        successRate:
          completed === 0 ? 0 : (runStats.succeeded || 0) / completed,
        runCount: runStats.total || 0,
        newJobs: allTimeNewJobs.count,
        activeJobs: activeJobs.count,
        jobsPerSuccessfulRun:
          runStats.avg_discovered === null
            ? 0
            : Math.round(runStats.avg_discovered * 10) / 10,
        zeroResultStreak,
        staleDurationHours:
          staleDurationHours === null
            ? null
            : Math.round(staleDurationHours * 10) / 10,
  ...
  public getProviderAnalytics(): ProviderAnalyticsRow[] {
    ...
    return providers.map((provider) => {
      ...
      const recentFailureTrend =
        completed >= 3 && failed / completed >= 0.5 ? 'degrading' : 'stable';

      // zero yield trend: count zero-yield succeeded runs in the last 24 hours
      const zeroYieldRunsResult = db
        .prepare(
          `
      ...
      const recentZeroYieldTrend =
        succeeded >= 3 && zeroYieldRuns / succeeded >= 0.75
          ? 'high-zero-yield'
          : 'stable';
      ...
      return {
        ...
        averageYield:
          runStats.avg_yield === null
            ? 0
            : Math.round(runStats.avg_yield * 10) / 10,
        recentFailureTrend,
