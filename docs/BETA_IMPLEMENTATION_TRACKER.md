...
| 16    | Adoption/flags telemetry                                                                                             | [x]    | Local-only adoption markers implemented: `adoption.installedAt` stamped at startup, `adoption.firstSourceAt` stamped on first source creation; stored in `app_settings` SQLite, never transmitted. No personal data. Markers module at `src/database/adoptionMarkers.ts`; backend stamp in `backend.ts`; source stamp in `app.ts`. NOT: `/api/adoption` endpoint or Settings UI (deferred per model incoherence). |
